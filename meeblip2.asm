...
;
;
;V??? - various reformatting, and replacement of some labels with local labels -- kjp
;V??? - Started using macros; no real code changes so I can compare the binary output -- kjp
...
DELTAT_H:	.byte 1		        ;/ of the main loop (1 bit = 32 microseconds)
...
; increase phase for DCO B *** must immediately follow deltaa
DELTAB_0: 	.byte 1
DELTAB_1: 	.byte 1
DELTAB_2: 	.byte 1
...
MIDICC:         .byte $80
	.equ MIDIMODWHEEL = MIDICC + 1
	.equ PWMDEPTH 	= MIDICC + 48
...
DELTA_C:	.dl	0x000D652E	            ; note  0 ( 8.175799 Hz) C
		.dl	0x000E3117	            ; note  1 ( 8.661957 Hz) C#
		.dl	0x000F091F	            ; note  2 ( 9.177024 Hz) D
		.dl	0x000FEE01	            ; note  3 ( 9.722718 Hz) D#
		.dl	0x0010E07F	            ; note  4 (10.300861 Hz) E
		.dl	0x0011E167	            ; note  5 (10.913382 Hz) F
		.dl	0x0012F197	            ; note  6 (11.562326 Hz) F#
		.dl	0x001411F6	            ; note  7 (12.249857 Hz) G
		.dl	0x0015437B	            ; note  8 (12.978272 Hz) G#
		.dl	0x0016872B	            ; note  9 (13.750000 Hz) A
		.dl	0x0017DE1A	            ; note 10 (14.567618 Hz) A#
		.dl	0x0019496D	            ; note 11 (15.433853 Hz) B
		.dl	0x001ACA5B	            ; note 12 (16.351598 Hz) C'
...
UART_RXC:
	...
	in r16, udr ;lds	r16, udr	; in	R16, UDR ; read received byte in R16
...
; note r16:r17 - whose idea was that? -- and why only here?
	sts	MIDIPBEND_L, R17	;\ 
...
LOAD_DELTA:
	...
	inc	R30
	rcall	LOAD_32BIT
	sub32   R16, R24	; replaces following four instructions
;	sub	R16, R24
;	sbc	R17, R25
;	sbc	R18, R26
;	sbc	R19, R27
...
;initialize Timer1:
	...
	out	TCCR1B, R16		;/ (clock = 32microseconds)
...
REZ_OVERFLOW_CHECK:
	...
	sbc	R23, R17	;/ (1 bit = 32 microseconds)
	sts16	TPREV_L, R18	; t0 = t
;	sts	TPREV_L, R18	;\ 
;	sts	TPREV_H, R19	;/ t0 = t
	sts16	DELTAT_L, R22	; R23:r22 = dT
;    	sts	DELTAT_L, R22	;\ 
...
MLP_LFOX:
	...
	sts16	LFO_FRAC_L, R19		; store LFO value
;	sts	LFO_FRAC_L, R19		;\ 
...
MLP_LFO2X:
	...
	sts16	LFO2_FRAC_L, R19	;\ store LFO2 value
;	sts	LFO2_FRAC_L, R19	;\ 
...
MLP_EDECAY:
        sub24   R19, R16		; R21:R20:R19 = L - dL
